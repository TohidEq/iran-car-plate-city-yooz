class YouzParser {
  ...
  parse(input) {
    ...
    while ((match = patternRegex.exec(input)) !== null) {
      ...
      const botResponses = match[2]
        .split("_")
        .map((response) => response.trim());
      if (userPattern.startsWith("{")) {
        // الگوی کلیدواژه‌ها
        const keywords = userPattern
          .slice(1, -1)
          .split("،")
    ...
    for (let pattern of this.patterns) {
      ...
      if (match) {
        ...
        if (response.endsWith("!>")) {
          response = this.getAdditionalResponses(
            response.slice(0, -99).trim(),
            cleanedMessage
          );
  ...
  getAdditionalResponses(initialResponse, userMessage) {
    ...
    for (let pattern of this.patterns) {
      ...
      if (match) {
        ...
        additionalResponses +=
          " " + this.resolveResponse(randomResponse, match);
      }
    }
